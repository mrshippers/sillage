...
function calcChemistry(selected) {
  ...
  if (Math.max(...frags.map(f => f.floral)) >= 5 && maxSmoky >= 6) clashBonus -= 3;
  ...
  const avgSp = frags.reduce((s, f) => s + f.spicy, 0) / frags.length;
  const avgFl = frags.reduce((s, f) => s + f.floral, 0) / frags.length;
  const opening = avgFresh >= 5 ? "Bright and lifted — the fresher notes lead" : avgSp >= 5 ? "Spiced and immediate — hits with intent" : "Warm and enveloping from the first breath";
  const heart = avgFl >= 4 ? "Floral complexity emerges as it settles" : avgWarm >= 7 ? "Rich amber and resinous warmth take centre stage" : "Woody depth anchors the middle hours";
...
// ── Selector Engine v3 ──
// Profile-driven scoring — uses actual scent characteristics, not just tag matching
...
function scoreFragrance(f, weather, occasion, energy, time) {
  ...
  // ── WEATHER (max ~35) — profile-driven ──
  if (weather.length) {
    maxScore += 35;
    const wKeys = weather.map(w => WEATHER_MAP[w]).filter(Boolean);
    const tagMatch = wKeys.filter(w => f.weather.includes(w)).length;
    // Tag match: 0-15 pts
    score += (tagMatch / wKeys.length) * 15;
    // Profile physics: 0-20 pts
    ...
    if (isHot) {
      ...
      if (f.projection >= 7) score -= 3; // heavy projection suffocates in heat
    }
    if (isCold) {
      score += f.warmth >= 8 ? 14 : f.warmth >= 6 ? 8 : f.warmth >= 4 ? 3 : 0;
      if (f.weight === "light" && f.warmth <= 4) score -= 8;
      if (f.longevity >= 8) score += 3; // cold kills projection, need staying power
    }
    if (isWet) {
      if (f.woody >= 6 || f.resinous >= 4) score += 4; // woods bloom in humidity
      if (f.projection <= 3) score -= 3; // skin scents vanish in rain
    }
    if (tagMatch === 0 && !isHot && !isCold) score -= 10; // no match at all
  }

  // ── OCCASION (max ~30) — profile + tag ──
  if (occasion) {
    maxScore += 30;
    const oKeys = OCCASION_MAP[occasion] || [];
    const oMatch = oKeys.filter(o => f.occasions.includes(o)).length;
    score += (oMatch / oKeys.length) * 12; // tag match: 0-12
    // Profile scoring per occasion
    if (occasion === "Date") {
      if (f.sweet >= 5) score += 4;
      if (f.warmth >= 6) score += 4;
      if (f.projection >= 5) score += 3;
      if (f.dark >= 5) score += 2;
      if (f.freshness >= 8 && f.warmth <= 3) score -= 3; // too clean for a date
    } else if (occasion === "Work / Office") {
      if (f.projection <= 5) score += 5; // don't choke colleagues
      if (f.dark <= 5) score += 3;
      if (f.freshness >= 5) score += 3;
      if (f.projection >= 8) score -= 6;
      if (f.sweet >= 7) score -= 3;
    } else if (occasion === "Night Out") {
      ...
      if (f.longevity >= 6) score += 3;
    } else if (occasion === "Casual / Weekend") {
  ...
  if (energy) {
    ...
    score += (eMatch / eKeys.length) * 10;
    // Profile synergy
    if (energy === "Romantic") { if (f.sweet >= 5) score += 3; if (f.warmth >= 6) score += 3; if (f.floral >= 3) score += 2; }
    if (energy === "Calm & Centred") { if (f.projection <= 5) score += 3; if (f.woody >= 5) score += 2; if (f.dark <= 5) score += 2; }
    if (energy === "Energetic") { if (f.freshness >= 6) score += 4; if (f.projection >= 5) score += 2; }
    if (energy === "Confident & Sharp") { if (f.projection >= 6) score += 3; if (f.longevity >= 7) score += 2; if (f.dark >= 4) score += 2; }
    if (energy === "Relaxed & Easy") { if (f.weight !== "heavy") score += 3; if (f.freshness >= 4) score += 2; if (f.projection <= 6) score += 2; }
    if (eMatch === 0) score -= 6;
  }
...
// ── Layer Partner — uses chemistry scoring ──
function getLayerPartner(primary, results) {
  // Exclude the primary and the alternatives already shown
  const resultIds = results.map(r => r.id);
  const candidates = FRAGRANCES.filter(f => !resultIds.includes(f.id));
  if (!candidates.length) return null;

  // Score each candidate on chemistry with the primary
  const scored = candidates.map(f => {
    const chem = calcChemistry([primary.id, f.id]);
    return { ...f, chemistry: chem ? chem.total : 0 };
  }).filter(c => c.chemistry >= 45) // minimum viable chemistry
...
export default function App() {
  ...
  const layerSuggestion = useMemo(() => selectorResults.length ? getLayerPartner(selectorResults[0], selectorResults) : null, [selectorResults]);
  ...
  const getScoreColor = (s) => s >= 75 ? "#7ab87a" : s >= 50 ? gold : s >= 30 ? "#c49a6a" : "#a06060";
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#08070605", color: "#e8e0d4", fontFamily: "'Cormorant Garamond', 'Georgia', serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 900, margin: "0 auto", padding: "0 24px" }}>
        {/* Header */}
        <header style={{ padding: "48px 0 28px", borderBottom: "1px solid rgba(201,169,110,0.1)", animation: mounted ? "fadeDown 0.8s ease both" : "none" }}>
        ...
        {/* Nav */}
        <nav style={{ display: "flex", gap: 0, padding: "20px 0", borderBottom: "1px solid rgba(255,255,255,0.03)", animation: mounted ? "fadeDown 0.8s ease 0.15s both" : "none", overflowX: "auto" }}>
          {MODES.map(m => (
        ...
        {mode === "perfumery" && (
          <div style={{ padding: "32px 0 60px", animation: "fadeUp 0.5s ease both" }}>
            ...
            <p style={{ fontSize: 12, color: "rgba(232,224,212,0.25)", fontFamily: sans, fontWeight: 300, margin: "0 0 28px", letterSpacing: "0.03em" }}>Pick 2–3 scents. See if they work together or fight each other.</p>
              ...
              <div style={{ marginTop: 40, animation: "fadeUp 0.6s ease both" }}>
                ...
                <div style={{ borderTop: "1px solid rgba(201,169,110,0.06)", paddingTop: 24, marginBottom: 24 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                    {[{ phase: "Opening", text: chemistry.opening, time: "0–15 min" }, { phase: "Heart", text: chemistry.heart, time: "15 min–3 hrs" }, { phase: "Dry-down", text: chemistry.drydown, time: "3 hrs+" }].map((p, i) => (
